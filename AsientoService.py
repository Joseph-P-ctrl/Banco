    ...
    def _conciliar_df( self, df_movimientos, df_asientos):
        ...
        try:
            if (len(self.df_movimientos.columns)<10):
               raise MyCustomException("Archivo moviento: Columnas no encontradas, elimine cabeceras innecesarias provecios ")
            if "Fecha" not in self.df_movimientos.columns:
                raise MyCustomException( "Archivo Estado de Cuenta: Columnas no encontradas")

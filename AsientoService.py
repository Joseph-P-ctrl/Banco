...
class AsientoService:
    ...
    def __init__(self):
        self.error= Error()
    def conciliar_df( self, df_movimientos, df_asientos):
    ...
    def conciliar( self, movimientosfile, asientosfile):
        try:
            ...
            self.df_movimientos = pd.read_excel(movimientosfile,   header=0 )
            print('resultado mov')
            self.df_asientos = pd.read_excel(asientosfile,   header=0 )
            print('asientos')        
            if (len(self.df_movimientos.columns)<10):
                self.error.message = "Archivo moviento: Columnas no encontradas, elimine cabeceras innecesarias provecios "
                return
